      ...
      <dl class="flex w-full flex-none justify-between gap-x-8 sm:w-auto">
        <div class="flex -space-x-0.5">
          ...
          <dd v-for="commenter in discussion.commenters" :key="commenter.id">
            <img class="size-6 rounded-full bg-gray-50 outline-2 outline-white dark:bg-gray-800 dark:outline-gray-900" :src="commenter.imageUrl" :alt="commenter.name" />
          </dd>
        </div>
        <div class="flex w-16 gap-x-2.5">
          <dt>
            <span class="sr-only">Total comments</span>
            <CheckCircleIcon v-if="discussion.status === 'resolved'" class="size-6 text-gray-400 dark:text-gray-500" aria-hidden="true" />
            <ChatBubbleLeftIcon v-else class="size-6 text-gray-400 dark:text-gray-500" aria-hidden="true" />
          </dt>
          <dd class="text-sm/6 text-gray-900 dark:text-white">{{ discussion.totalComments }}</dd>
        </div>
      </dl>
    </li>
  </ul>
