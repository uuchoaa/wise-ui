<template>
  <ul role="list" class="divide-y divide-gray-100 dark:divide-white/5">
    <li v-for="comment in comments" :key="comment.id" class="flex gap-x-4 py-5">
      <img class="size-12 flex-none rounded-full bg-gray-50 dark:bg-gray-800 dark:outline dark:-outline-offset-1 dark:outline-white/10" :src="comment.imageUrl" alt="" />
      <div class="flex-auto">
        <div class="flex items-baseline justify-between gap-x-4">
          <p class="text-sm/6 font-semibold text-gray-900 dark:text-white">{{ comment.name }}</p>
          <p class="flex-none text-xs text-gray-600 dark:text-gray-400">
            <time :datetime="comment.dateTime">{{ comment.date }}</time>
          </p>
        </div>
        <p class="mt-1 line-clamp-2 text-sm/6 text-gray-600 dark:text-gray-400">{{ comment.content }}</p>
      </div>
    </li>
  </ul>
</template>
